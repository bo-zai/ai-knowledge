...
function computeConfidence(anchor: TableAnchor): number {
  ...
  // 多入口覆盖（0 - 0.15）
  const entryPointTypes = new Set(anchor.traceSources.flatMap(s => s.entryPoints.map(e => e.kind)));
  const multiEntryPointBonus = Math.min(0.15, entryPointTypes.size * 0.05);

  // 表关联密度（0 - 0.1）
  const tableRelationBonus = anchor.relatedTables?.length > 0
    ? Math.min(0.1, anchor.relatedTables.length * 0.02)
    : 0;

  return Math.min(1, traceDepthScore + crossModuleBonus + multiEntryPointBonus + tableRelationBonus);
}
```
