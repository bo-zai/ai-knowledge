...
describe("knowledge-read-runtime (LangGraph + LLM)", () => {
  it(
    "LLM 使用 read_file_window 工具读取文件",
    async () => {
      const result = await runKnowledgeReadRuntime({
        repoPath: REPO_PATH,
        instruction:
          "使用 read_file_window 工具读取 vitest.config.ts 文件的第 1-10 行。返回 JSON 格式结果，包含 answer、evidence_refs、insufficient_evidence 字段。",
  ...
  it(
    "LLM 使用 search_repo_text 工具搜索代码",
    async () => {
      const result = await runKnowledgeReadRuntime({
        repoPath: REPO_PATH,
        instruction:
          "使用 search_repo_text 工具搜索 DEFAULT_KNOWLEDGE_DIR 常量。返回 JSON 格式结果。",
        model: LLM_DEFAULTS.model,
        baseUrl: LLM_DEFAULTS.baseUrl,
        apiKey: LLM_DEFAULTS.apiKey,
        limits: {
