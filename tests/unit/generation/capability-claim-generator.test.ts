...
describe("CandidateClaimSchema", () => {
  it("accepts valid CAP claim", () => {
    const claim = CandidateClaimSchema.parse({
      suggestedType: "CAP",
      claimText:
        "DB knowledge generation capability transforms MyBatis evidence into DB objects",
      confidence: "high",
      evidenceRefs: [
        "evidence://behavior/BEH-001",
        "evidence://contract/CON-EVID-001",
      ],
  ...
  it("accepts structured fieldSemantics values from real LLM output", () => {
    const result = CandidateClaimSchema.safeParse({
      suggestedType: "CON",
      claimText:
        "Order detail contract exposes goods line items and goods price semantics.",
      confidence: "high",
  ...
  it("accepts valid OPEN claim without evidence refs", () => {
    const claim = CandidateClaimSchema.parse({
      ...
      blockedDecisions: [
        "Cannot decide if inferred descriptions are acceptable",
      ],
    });
    ...
    const claims: CandidateClaim[] = [
      {
        ...
        objectHints: {
          minimalNextEvidence: ["Find field description documentation"],
        },
      },
    ];
...
describe("buildSkeletonClaims", () => {
  it("generates CAP, FLOW, MOD, CON, VER, OPEN claims from minimal bundle", () => {
    const bundle: EvidenceBundle = {
      ...
      flowTraces: [
        {
          ...
          steps: [{ action: "parse", outcome: "ast" }],
        },
      ],
      behaviorSlices: [],
      dataContracts: [
      ...
      moduleSurfaces: [
        {
          ref: "evidence://module/MOD-001",
          rootPath: "src/generation",
          exports: ["generate"],
          responsibilities: ["DB generation"],
        },
      ],
      validationAnchors: [
        {
      ...
      openQuestions: [
        {
          ...
          blockedDecisions: ["field-description-source"],
          minimalNextEvidence: "doc",
        },
      ],
    };
  ...
  it("uses capabilityHints.nameCandidates[0] as capability name", () => {
    ...
    expect(capClaim?.objectHints?.canonicalTerm).toBe(
      "MyBatis Mapper Knowledge",
    );
  });
  ...
  it("generates OPEN claims from openQuestions", () => {
      ...
      openQuestions: [
        {
          ...
          blockedDecisions: ["source-decision"],
          minimalNextEvidence: "doc",
        },
        {
          id: "Q-002",
          question: "How to handle edge case?",
          blockedDecisions: ["edge-case-decision"],
          minimalNextEvidence: "test",
        },
      ],
    };
    ...
    expect(openClaims.some((c) => c.claimText === "What is the source?")).toBe(
      true,
    );
    expect(
      openClaims.some((c) => c.claimText === "How to handle edge case?"),
    ).toBe(true);
  ...
  it("uses highest-relevance evidence from sorted bundle (business over AOP)", () => {
    ...
    const bundle: EvidenceBundle = {
      ...
      risks: [],
      capabilityHints: {
        nameCandidates: ["Course Management"],
        relatedTerms: ["course", "mybatis"],
      },
      ...
      entryPoints: [
        {
          ref: "evidence://entry/EP-001",
          kind: "http",
          location: "src/main/java/com/demo/controller/CourseController.java",
          name: "getCourse",
          targetRelevance: 0.9,
        },
        {
          ref: "evidence://entry/EP-002",
          kind: "service",
          location: "src/main/java/com/demo/aop/LogAop.java",
          name: "logBefore",
          targetRelevance: 0.3,
        },
      ],
      ...
      dataContracts: [
        {
          ref: "evidence://contract/CON-001",
          kind: "sql",
          location: "src/main/resources/mapper/CourseMapper.xml",
          name: "selectCourse",
          targetRelevance: 0.85,
        },
        {
          ref: "evidence://contract/CON-002",
          kind: "interface",
          location: "src/main/java/com/demo/aop/RateLimitAspect.java",
          name: "RateLimit",
          targetRelevance: 0.2,
        },
      ],
      ...
      moduleSurfaces: [
        {
          ref: "evidence://module/MOD-001",
          rootPath: "src/main/java/com/demo/controller",
          exports: ["CourseController"],
          responsibilities: [],
          targetRelevance: 0.8,
        },
        {
          ref: "evidence://module/MOD-002",
          rootPath: "src/main/java/com/demo/aop",
          exports: ["LogAop"],
          responsibilities: [],
      ...
      validationAnchors: [
        {
          ref: "evidence://validation/VAL-001",
          kind: "test",
          location: "src/test/java/com/demo/service/CourseServiceTest.java",
          name: "shouldLoadCourse",
          targetRelevance: 0.75,
        },
        {
          ref: "evidence://validation/VAL-002",
          kind: "test",
          location: "src/test/java/com/demo/aop/AopTest.java",
          name: "aopTest",
          targetRelevance: 0.2,
        },
      ],
    ...
    expect(modClaim?.objectHints?.modulePath).toBe(
  ...
  it("generates TERM claims from matched business evidence", () => {
    ...
    expect(termClaims.map((claim) => claim.objectHints?.canonicalTerm)).toEqual(
      expect.arrayContaining(["goods", "order"]),
    );
    expect(termClaims.every((claim) => claim.evidenceRefs.length > 0)).toBe(
      true,
    );
