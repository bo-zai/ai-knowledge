...
describe("FileBackend", () => {
  ...
  describe("ls", () => {
    ...
    it("使用虚拟路径模式", async () => {
      ...
      const virtualBackend = new FileBackend({
        rootDir: testDir,
        virtualMode: true,
      });
  ...
  describe("write_file", () => {
    ...
    it("自动创建父目录", async () => {
      const result = await backend.write_file(
    ...
    it("拒绝过大的内容", async () => {
      const smallBackend = new FileBackend({
        rootDir: testDir,
        maxFileSizeMb: 0.001,
      });
  ...
  describe("edit_file", () => {
    ...
    it("找不到字符串时返回错误", async () => {
      ...
      const result = await backend.edit_file(
        "edit.txt",
        "notfound",
        "replacement",
      );
  ...
  describe("grep", () => {
    ...
    it("在目录中搜索", async () => {
      ...
      await fs.writeFile(
        path.join(testDir, "sub", "nested.txt"),
        "hello nested",
      );
  ...
  describe("路径安全", () => {
    ...
    it("虚拟模式阻止路径遍历", async () => {
      ...
      await expect(
        virtualBackend.read_file("/../etc/passwd"),
      ).rejects.toThrow();
    });
