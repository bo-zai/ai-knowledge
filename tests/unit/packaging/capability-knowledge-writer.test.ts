...
describe("buildCapabilityKnowledgeFiles", () => {
  ...
  it("generates object files under objects/<type-dir>/", () => {
    ...
    const capFile = files.find((f) =>
      f.path.includes("objects/capabilities/CAP-DB-KNOWLEDGE-GENERATION"),
    );
  ...
  it("generates capability view under views/capabilities/", () => {
    ...
    const viewFile = files.find(
      (f) => f.path === "views/capabilities/CAP-DB-KNOWLEDGE-GENERATION.md",
    );
  ...
  it("generates primary capability Markdown under capabilities/", () => {
    ...
    const primaryFile = files.find(
      (f) => f.path === "capabilities/CAP-DB-KNOWLEDGE-GENERATION.md",
    );
  ...
  it("catalog includes capability routing mapping", () => {
    ...
    expect(catalog).toContain(
      "view: views/capabilities/CAP-DB-KNOWLEDGE-GENERATION.md",
    );
...
describe("evidence index", () => {
  ...
  const evidenceIndex: EvidenceIndexItem[] = [
    ...
    },
    {
      ...
      kind: "flow",
      name: "flow trace",
      summary: "getCourse -> selectCourse",
    ...
    },
    {
      ...
      kind: "module",
      location: "src/main/java/com/demo/controller",
      name: "src/main/java/com/demo/controller",
...
describe("report with LangGraph metadata", () => {
  ...
  it("includes llmRuntime and graph in report JSON", () => {
    ...
    const reportFile = files.find(
      (f) => f.path === "reports/capability-generation.json",
    );
  ...
  it("includes objectSourceCounts and requiredBusinessObjects in report", () => {
    ...
    const report = JSON.parse(reportFile!.content);
    expect(report.objectSourceCounts).toEqual({
      llm: 3,
      skeleton: 2,
      evidence_seed: 1,
    });
  ...
  it("includes graphTrace in debug JSON", () => {
    ...
    const debugFile = files.find(
      (f) => f.path === "debug/capability-llm-request.json",
    );
...
describe("writeCapabilityKnowledgePackage", () => {
  it("removes stale files from previous ai-knowledge output before writing", async () => {
    const outputRoot = await mkdtemp(
      join(tmpdir(), "capability-clean-output-"),
    );
    ...
    await expect(
      access(join(outputRoot, "ai-knowledge", "catalog.yaml")),
