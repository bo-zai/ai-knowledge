...
describe('multi-model-config', () => {
  ...
  describe('resolveApiKey', () => {
    ...
    it('解析嵌入环境变量的字符串', () => {
      ...
      expect(resolveApiKey('${PREFIX}-middle-${SUFFIX}')).toBe('prefix-middle-suffix');
    });
  ...
  describe('loadMultiModelsFile', () => {
    ...
    it('文件不存在时返回空配置', async () => {
      const result = await loadMultiModelsFile(join(testDir, 'nonexistent.json'));
    ...
    it('models 字段不是数组时返回空配置', async () => {
      ...
      await writeFile(configPath, JSON.stringify({ models: 'not-array' }), 'utf8');
  ...
  describe('selectModelForTask', () => {
    ...
    it('使用指定模型 ID', () => {
      const configFile: MultiModelsFile = {
        models: [],
        routing: {
          rules: [{ taskType: 'generation', preferredModelId: 'premium-model' }],
        },
      };

      const result = selectModelForTask(configFile, 'generation', createValidModels());
    ...
    it('使用偏好层级', () => {
      ...
      const result = selectModelForTask(configFile, 'analysis', createValidModels());
    ...
    it('使用默认模型', () => {
      ...
      const result = selectModelForTask(configFile, 'unknown-task', createValidModels());
