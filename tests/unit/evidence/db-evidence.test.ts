...
import { mergeDbFieldSources, extractTableSchema, mergeSchemaSources } from '../../../src/evidence/db-evidence';
...
describe('mergeDbFieldSources', () => {
  ...
  it('keeps first field when both have same source', () => {
    const fields = mergeDbFieldSources([
      { name: 'name', type: 'varchar', nullable: true, default: null, description_zh: '名称', description_source: 'inferred', constraints: [] },
      { name: 'name', type: 'varchar', nullable: true, default: null, description_zh: '姓名', description_source: 'inferred', constraints: [] },
    ]);
...
CREATE TABLE orders (
  ...
  total DECIMAL(10, 2)
    ...
    const inferredGaps = result.gaps.filter((g) => g.kind === 'inferred-description');
  ...
  it('emits gap when only inferred source available', () => {
    const result = extractTableSchema({
      ...
      sources: [],
      inferredFields: [
        { name: 'id', type: 'bigint', nullable: false, default: null, description_zh: '推断字段', description_source: 'inferred', constraints: [] },
      ],
    });
