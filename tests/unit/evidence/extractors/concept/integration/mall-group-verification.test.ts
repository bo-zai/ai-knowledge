...
import type { ConceptTracePath, TableAnchor, ConceptCandidate } from '../../../../src/evidence/extractors/concept/types.js';
...
describe('mall-group integration verification', () => {
  ...
  beforeAll(() => {
    // 加载 fixture 数据
    const fixturePath = join(__dirname, '..', 'fixtures', 'mall-group-trace-path.json');
  ...
  describe('entry point validation', () => {
    ...
    it('all entry points are controller type', () => {
      const entryPoints = tableAnchor.traceSources.flatMap(s => s.entryPoints);
  ...
  describe('service chain validation', () => {
    ...
    it('has ProductService in service chain', () => {
      const tracePath = fixture.tracePath;
      const serviceNames = tracePath.serviceChain?.map(s => s.className) || [];
 ...
 */
function buildTableAnchorFromTracePath(tracePath: ConceptTracePath): TableAnchor {
