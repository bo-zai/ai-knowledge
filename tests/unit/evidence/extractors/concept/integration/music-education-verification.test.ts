...
describe('music-education integration verification', () => {
  ...
  beforeAll(() => {
    // 加载 fixture 数据
    const fixturePath = join(__dirname, '..', 'fixtures', 'music-education-trace-path.json');
  ...
  describe('pathway statistics', () => {
    it('has controller pathway in music-course', () => {
      const controllerSources = tableAnchor.traceSources.filter(
        s => s.entryPoints.some(e => e.kind === 'controller')
      );
      expect(controllerSources.length).toBeGreaterThan(0);
    });

    it('has scheduled pathway in music-sync', () => {
      const scheduledSources = tableAnchor.traceSources.filter(
        s => s.entryPoints.some(e => e.kind === 'scheduled')
      );
  ...
  describe('confidence calculation', () => {
    ...
    it('receives cross-module bonus in breakdown', () => {
      expect(candidate.confidenceBreakdown.crossModule).toBe(CROSS_MODULE_BONUS);
    });
  ...
  describe('candidate validation', () => {
    ...
    it('has valid confidence breakdown', () => {
      expect(candidate.confidenceBreakdown.traceDepth).toBeGreaterThanOrEqual(0);
      expect(candidate.confidenceBreakdown.crossModule).toBe(CROSS_MODULE_BONUS);
      expect(candidate.confidenceBreakdown.multiEntryPoint).toBeGreaterThanOrEqual(0);
      expect(candidate.confidenceBreakdown.tableRelation).toBeGreaterThanOrEqual(0);
    });
  ...
  describe('module coverage validation', () => {
    ...
    it('music-course has controller entry points', () => {
      ...
      const entryKinds = musicCourseSource?.entryPoints.map(e => e.kind) || [];
  ...
  describe('service chain validation', () => {
    it('music-course has CourseService', () => {
      const musicCoursePath = fixture.tracePaths.find(tp => tp.source === 'music-course');
      const serviceNames = musicCoursePath?.tracePath.serviceChain?.map(s => s.className) || [];
      expect(serviceNames).toContain('CourseService');
    });

    it('music-sync has CourseSyncService', () => {
      const musicSyncPath = fixture.tracePaths.find(tp => tp.source === 'music-sync');
      const serviceNames = musicSyncPath?.tracePath.serviceChain?.map(s => s.className) || [];
 ...
 */
function buildCrossModuleTableAnchor(tracePathEntries: TracePathEntry[]): TableAnchor {
  ...
  // 计算聚合置信度
  const avgConfidence = traceSources.reduce((sum, s) => sum + s.confidence, 0) / traceSources.length;
  const crossModuleBonus = moduleNames.length > 1 ? CROSS_MODULE_BONUS : 0;

  // 计算多入口类型加成
  const entryKinds = new Set(traceSources.flatMap(s => s.entryPoints.map(e => e.kind)));
  const multiEntryPointBonus = Math.min(0.15, (entryKinds.size - 1) * 0.05);

  const aggregatedConfidence = Math.min(1.0, avgConfidence + crossModuleBonus + multiEntryPointBonus);
 ...
 */
function buildCandidateFromTableAnchor(anchor: TableAnchor, crossModuleBonus: number): ConceptCandidate {
  const entryPoints = anchor.traceSources.flatMap(s => s.entryPoints);
  const entryKinds = new Set(entryPoints.map(e => e.kind));
  const avgTraceConfidence = anchor.traceSources.reduce((sum, s) => sum + s.confidence, 0) / anchor.traceSources.length;
