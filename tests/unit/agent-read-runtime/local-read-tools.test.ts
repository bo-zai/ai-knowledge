...
describe('local read tool handlers', () => {
  ...
  it('rejects absolute path outside repo', async () => {
    ...
    const result = await handlers.readFileWindow({ path: '/etc/passwd', startLine: 1, endLine: 1 });
  ...
  it('returns no matches for non-existent text', async () => {
    ...
    const result = await handlers.searchRepoText({ query: 'nonexistent', limit: 10 });
  ...
  it('reads symbol definition by searching for function keyword', async () => {
    ...
    const result = await handlers.readSymbolDefinition({ symbol: 'saveOrder', limit: 10 });
  ...
  it('reads symbol references', async () => {
    ...
    const result = await handlers.readSymbolReferences({ symbol: 'saveOrder', limit: 10 });
  ...
  it('rejects invalid line window', async () => {
    ...
    const result = await handlers.readFileWindow({ path: 'src/sample.ts', startLine: 5, endLine: 3 });
  ...
  it('rejects line window exceeding limit', async () => {
    ...
    const result = await handlers.readFileWindow({ path: 'src/sample.ts', startLine: 1, endLine: 10 });
