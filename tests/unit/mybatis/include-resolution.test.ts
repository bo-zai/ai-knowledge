...
describe('include resolution', () => {
  ...
  it('resolves Base_Column_List for music_user table', async () => {
    ...
    const stmt = mapper?.statements.find((item) => item.id === 'getAppUserById');
    expect(stmt).toBeTruthy();
...
describe('resultMap extraction', () => {
  it('extracts resultMap definitions from CategoryMapper', async () => {
    ...
    const baseCategoryResultMap = mapper?.resultMaps.find((rm) => rm.id === 'BaseCategoryResultMap');
    expect(baseCategoryResultMap).toBeTruthy();
    expect(baseCategoryResultMap?.type).toBe('com.education.music.core.DO.mall.CategoryDO');
    expect(baseCategoryResultMap?.mappings.length).toBeGreaterThan(0);

    // Check specific mappings
    const nameMapping = baseCategoryResultMap?.mappings.find((m) => m.property === 'name');
    expect(nameMapping?.column).toBe('name');

    const picUrlMapping = baseCategoryResultMap?.mappings.find((m) => m.property === 'picUrl');
  ...
  it('extracts resultMap definitions from UserMapper', async () => {
    ...
    const baseResultMap = mapper?.resultMaps.find((rm) => rm.id === 'BaseResultMap');
    expect(baseResultMap).toBeTruthy();
    expect(baseResultMap?.type).toBe('com.education.music.core.DO.user.UserDO');

    // Check specific mappings
    const mobileMapping = baseResultMap?.mappings.find((m) => m.property === 'mobile');
    expect(mobileMapping?.column).toBe('mobile');

    const nicknameMapping = baseResultMap?.mappings.find((m) => m.property === 'nickname');
...
describe('statement resultType/resultMap preservation', () => {
  it('preserves resultType for auth_menu statements', async () => {
    ...
    const stmt = mapper?.statements.find((item) => item.id === 'getMenuAuthList');
    expect(stmt).toBeTruthy();
