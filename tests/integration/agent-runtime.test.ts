...
describe('Agent Runtime Integration', () => {
      ...
      it('parses valid JSON output into correct structure', () => {
        const result = validateFinalOutput({
          finalText: JSON.stringify({
            ...
            evidence_refs: [
              { file: 'src/order.ts', start_line: 10, end_line: 20, note: 'Main logic' },
            ],
            insufficient_evidence: false,
          }),
          repairAttempts: 0,
        ...
        expect(result.parsedOutput?.answer).toBe('The function processes orders.');
        expect(result.parsedOutput?.evidence_refs).toHaveLength(1);
        expect(result.parsedOutput?.evidence_refs[0]?.file).toBe('src/order.ts');
      ...
      it('handles multiple evidence refs correctly', () => {
        const result = validateFinalOutput({
          finalText: JSON.stringify({
            ...
            evidence_refs: [
              { file: 'src/a.ts', start_line: 1, end_line: 5, note: 'Source A' },
              { file: 'src/b.ts', start_line: 10, end_line: 15, note: 'Source B' },
              { file: 'src/c.ts', start_line: 20, end_line: 25, note: 'Source C' },
            ],
            insufficient_evidence: false,
          }),
          repairAttempts: 0,
  ...
  describe('File Operations Integration', () => {
    ...
    beforeEach(async () => {
      // 创建临时测试目录结构
      tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-runtime-integration-'));
    ...
    describe('readFileWindow', () => {
      it('reads file window with correct line numbers', async () => {
        ...
        expect(result).toContain('2 |   private users: Map<string, User> = new Map();');
      ...
      it('reads middle section of a file', async () => {
        ...
        expect(result).toContain('4 |   async findById(id: string): Promise<User | undefined> {');
  ...
  describe('JSON Output Parsing Integration', () => {
    describe('parseKnowledgeReadAgentOutput', () => {
      ...
      it('repairs trailing commas in JSON', () => {
        const output = JSON.stringify({
          answer: 'Test',
          evidence_refs: [{ file: 'a.ts', start_line: 1, end_line: 2, note: 'note' }],
          insufficient_evidence: false,
        }).replace('}', ',}');
      ...
      it('throws error for completely invalid output', () => {
        expect(() => parseKnowledgeReadAgentOutput('random text without json'))
          .toThrow('Agent output is not valid JSON');
      });

      it('throws error for missing required fields', () => {
        expect(() => parseKnowledgeReadAgentOutput(JSON.stringify({ answer: 'only answer' })))
          .toThrow();
      });
    });
  ...
  describe('Full Graph Execution Integration', () => {
    ...
    beforeEach(async () => {
      repoPath = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-graph-integration-'));
        ...
        model: createFakeModel([
          ...
          new AIMessage({
            content: JSON.stringify({
              ...
              evidence_refs: [
                { file: 'src/sample.ts', start_line: 1, end_line: 1, note: 'hello function' },
                { file: 'src/sample.ts', start_line: 2, end_line: 2, note: 'add function' },
              ],
              insufficient_evidence: false,
            }),
          }),
  ...
  describe('Error Handling Integration', () => {
    ...
    beforeEach(async () => {
      repoPath = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-error-handling-'));
      budget = createBudgetState(resolveKnowledgeReadLimits());
