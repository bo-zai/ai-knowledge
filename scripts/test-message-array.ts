...
async function main() {
  ...
  const messages: LlmMessage[] = [
    { role: 'system', content: '你是一个 JSON 数据生成助手。只输出纯 JSON，不要任何其他文字。' },
    { role: 'user', content: '生成一个简单的用户信息对象，包含 name 和 age 字段。' },
  ];
  ...
  const multiTurnMessages: LlmMessage[] = [
    { role: 'system', content: '你是一个 JSON 数据生成助手。只输出纯 JSON，不要任何其他文字。' },
    { role: 'user', content: '生成一个产品信息对象，包含 product_name、price、category 字段。' },
    { role: 'assistant', content: '好的，我来生成产品信息：\n```json\n{\n  "product_name": "智能手表",\n  "price": 299,\n  "category": "电子产品"\n}\n```' },
    { role: 'user', content: '你的输出被 markdown 代码块包裹了。请直接输出纯 JSON，去掉 ```json 和 ``` 标记。' },
  ];
  ...
  try {
    const result3 = await callLlmForJson({
      // 使用 message 数组模式
      messages: [
        { role: 'system', content: '你是一个知识生成助手。输出纯 JSON 数组，不要 markdown 包裹。' },
        { role: 'user', content: '生成一个概念对象数组，包含 concept_name 和 summary_zh 字段。示例：\n[{"concept_name": "用户", "summary_zh": "系统使用者"}]' },
      ],
