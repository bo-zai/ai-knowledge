...
try {
  ...
  for (const p of previousIndex.partitions) {
    console.log(`  - ${p.partitionId}: ${p.tableCount} tables, ${p.entryPointCount} entry points`);
  }
} catch (err) {
...
if (previousIndex) {
  ...
  console.log(`Partition count: ${previousCount} → ${newCount} (Δ ${newCount - previousCount})`);
  ...
  for (const p of result.partitions) {
    const anchorTable = p.tables.find(t => t.role === 'primary')?.tableName ?? 'unknown';
