 ...
 * MQ Consumer 路径发现配置
 */
export interface MqConsumerPathDiscoveryConfig {
  /** 最大追溯深度（Service 链层数） */
...
export class MqConsumerPathDiscovery {
  ...
  private readonly config: Required<MqConsumerPathDiscoveryConfig>;

  /**
   * 创建 MqConsumerPathDiscovery 实例
   *
   * @param adapter - 语言适配器
  ...
  async discover(): Promise<DiscoveryPathResult> {
    ...
    const mqConsumerEntryPoints: EntryPointInfo[] = [];

    try {
      // 1. 检测所有入口点
      const allEntryPoints = await this.adapter.detectEntryPoints(this.modulePath);

      // 2. 过滤只保留 MQ Consumer 入口点
      for (const ep of allEntryPoints) {
        if (ep.kind === 'mq_consumer') {
          mqConsumerEntryPoints.push(ep);
        }
      }

      // 3. 为每个 MQ Consumer 入口点追溯完整链路
      for (const entryPoint of mqConsumerEntryPoints) {
        try {
          const tracePath = await this.traceMqConsumerPath(entryPoint, errors);
          if (tracePath) {
            tracePaths.push(tracePath);
          }
        } catch (error) {
          const errorMsg = error instanceof Error ? error.message : String(error);
          errors.push(`追溯 MQ Consumer ${entryPoint.className} 失败: ${errorMsg}`);
        }
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
    ...
    return {
      pathway: 'mq_consumer',
      entryPoints: mqConsumerEntryPoints,
      tracePaths,
      errors,
    };
  }
  ...
  /**
   * 追溯单个 MQ Consumer 的完整路径
   */
  private async traceMqConsumerPath(
   ...
   * 追溯 Service 调用链
   *
   * 从 MQ Consumer 追溯到 Service 层，支持多级 Service 调用
   */
  ...
  private async traceDeeperServices(
    ...
    const deeperServices = await this.adapter.traceToService({
      kind: 'mq_consumer', // 使用 mq_consumer 类型，但实际上只是复用追溯逻辑
