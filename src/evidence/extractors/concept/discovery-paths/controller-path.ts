 ...
 * Controller 路径发现配置
 */
export interface ControllerPathDiscoveryConfig {
  /** 最大追溯深度（Service 链层数） */
...
export class ControllerPathDiscovery {
  ...
  private readonly config: Required<ControllerPathDiscoveryConfig>;

  /**
   * 创建 ControllerPathDiscovery 实例
   *
   * @param adapter - 语言适配器
  ...
  async discover(): Promise<DiscoveryPathResult> {
    ...
    const controllerEntryPoints: EntryPointInfo[] = [];

    try {
      // 1. 检测所有入口点
      const allEntryPoints = await this.adapter.detectEntryPoints(this.modulePath);

      // 2. 过滤只保留 Controller 入口点
      for (const ep of allEntryPoints) {
        if (ep.kind === 'controller') {
          controllerEntryPoints.push(ep);
        }
      }

      // 3. 为每个 Controller 入口点追溯完整链路
      for (const entryPoint of controllerEntryPoints) {
        try {
          const tracePath = await this.traceControllerPath(entryPoint, errors);
          if (tracePath) {
            tracePaths.push(tracePath);
          }
        } catch (error) {
          const errorMsg = error instanceof Error ? error.message : String(error);
          errors.push(`追溯 Controller ${entryPoint.className} 失败: ${errorMsg}`);
        }
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
    ...
    return {
      pathway: 'controller',
      entryPoints: controllerEntryPoints,
      tracePaths,
      errors,
    };
  }
  ...
  /**
   * 追溯单个 Controller 的完整路径
   */
  private async traceControllerPath(
   ...
   * 追溯 Service 调用链
   *
   * 从 Controller 追溯到 Service 层，支持多级 Service 调用
   */
  ...
  private async traceDeeperServices(
    ...
    const deeperServices = await this.adapter.traceToService({
      kind: 'controller', // 使用 controller 类型，但实际上只是复用追溯逻辑
