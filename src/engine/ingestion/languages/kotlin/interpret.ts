...
export function interpretKotlinImport(
  ...
  switch (kind) {
    ...
    case "wildcard":
      return {
        kind: "wildcard",
        targetRaw: source.endsWith(".*") ? source : `${source}.*`,
      };
    default:
      return null;
  }
...
export function interpretKotlinTypeBinding(
  captures: CaptureMatch,
...
export function normalizeKotlinType(text: string): string {
  ...
  const map = out.match(
    /^(?:Map|MutableMap|HashMap|LinkedHashMap)<[^,<>]+,\s*([^,<>]+)>$/,
  );
