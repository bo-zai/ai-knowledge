...
import type { LbugValue, Connection, QueryResult, Database } from '@ladybugdb/core';
...
export async function ensureBridgeSchema(handle: BridgeHandle): Promise<void> {
  const conn = handle._conn as Connection;
 ...
 */
function unwrapQueryResult(queryResult: QueryResult | QueryResult[]): QueryResult {
...
export async function closeBridgeDb(handle: BridgeHandle): Promise<void> {
  ...
  try {
    await (handle._conn as Connection).query('CHECKPOINT');
  } catch {
    /* ignore — older LadybugDB or schemaless DB may not accept it */
  }
  try {
    await (handle._conn as Connection).close();
  } catch {
    /* ignore */
  }
  try {
    await (handle._db as Database).close();
  } catch {
    /* ignore */
  }
}
