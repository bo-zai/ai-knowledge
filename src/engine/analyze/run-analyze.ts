...
export async function runFullAnalysis(
  ...
  if (shouldLoadCache && existingMeta) {
    try {
      ...
      // Don't close here — keep the connection open for Phase 2.
      // On Windows, LadybugDB doesn't release file handles immediately,
      // so closing and reopening causes "Access is denied" errors.
    } catch (err: any) {
      log(
  ...
  // file handles immediately after close(), so closing/reopening fails.
  await initLbug(lbugPath);
  await clearAllData();
  try {
