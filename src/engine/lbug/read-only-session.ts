...
import {
  ...
  initLbug,
  initLbugWithDb,
} from './pool-adapter.js';

export type ReadOnlyQueryExecutor = (cypher: string) => Promise<Record<string, unknown>[]>;

function getReadOnlyRepoId(dbPath: string): string {
  return `read-only:${path.resolve(dbPath)}`;
}

export async function initReadOnlyLbugWithDb(dbPath: string, db: Database): Promise<void> {
