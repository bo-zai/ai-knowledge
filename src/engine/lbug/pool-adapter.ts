...
interface PoolEntry {
  ...
  waiters: Array<(conn: Connection) => void>;
  lastUsed: number;
  dbPath: string;
...
function createConnection(db: Database): Connection {
  silenceStdout();
  try {
 ...
 * Retries on lock errors (e.g., when `knowledge analyze` is running).
 *
...
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`LadybugDB not found at ${dbPath}. Run: knowledge analyze`);
  }
  ...
  if (!shared) {
    ...
    // avoids lock conflicts when `knowledge analyze` is writing.
 ...
 */
function checkout(entry: PoolEntry): Promise<Connection> {
 ...
 */
function checkin(entry: PoolEntry, conn: Connection): void {
  if (entry.closed) {
