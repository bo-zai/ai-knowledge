 ...
 * 上下文字段到模板变量的映射
 */
export const CONTEXT_FIELD_MAPPING: Record<
  KnowledgeType,
    ...
    if (templateKey) {
      ...
      if (templateKey === "aliases" && typeof templateVar === "string") {
        ...
        if (typeof templateVar === "string") {
          result[templateKey] = (result[templateKey] as string).replace(
            placeholder,
            templateVar,
          );
        }
      } else if (
        Array.isArray(result[templateKey]) &&
        typeof templateVar === "string"
      ) {
        // 数组字段：替换占位符元素
