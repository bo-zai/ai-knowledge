...
function calculateBackoffMs(attempt: number): number {
  const backoffMs =
    BACKOFF_CONFIG.baseMs * Math.pow(BACKOFF_CONFIG.multiplier, attempt - 1);
 ...
 * 尝试解析 JSON
 */
function tryParseJson(cleanedText: string): {
  success: boolean;
  data?: unknown;
  error?: Error;
} {
...
export async function callLlmForJson<T = Record<string, unknown>>(
  ...
  try {
    ...
    const isTimeout =
      errorMsg.includes("timeout") || errorName.includes("Timeout");
  ...
  while (attempt < maxRetries) {
    ...
    const retrySystemPrompt = getRetrySystemPrompt(
      attempt,
    ...
    try {
      ...
      logger.debug(
        `${logLabel}: 第${attempt}次重试完成，耗时 ${callResult.durationMs}ms`,
      );
      ...
      logger.error(
        `${logLabel}: 第${attempt}次重试异常 (${errorName}): ${detailedMsg}`,
      );
    }
  }
...
export function generateBatchStatsReport(
  ...
  logger.info(
    `首次成功: ${stats.successCount - stats.retrySuccessCount - stats.fallbackCount}`,
  );
