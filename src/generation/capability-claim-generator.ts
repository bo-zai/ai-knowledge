...
export function buildCapabilityClaimPrompt(bundle: EvidenceBundle): string {
  ...
  lines.push('suggestedType (CAP|TERM|FLOW|MOD|CON|VER|OPEN), claimText (string), confidence (high|medium|low),');
  lines.push('evidenceRefs (string[]), decisionPoints (string[]), sddStageUses (string[]),');
  lines.push('unsupportedParts (string[]), blockedDecisions (string[]), objectHints (object, optional).');
  lines.push('');
  lines.push('objectHints fields by type:');
  lines.push('- ALL: canonicalTerm, subject, businessDefinition, aliases, notEqualTo');
  lines.push('- CAP: goal, successCriteria, nonGoals');
  lines.push('- FLOW: orderedSteps[{action, evidenceRef?, note}], failureBranches, compensation');
  lines.push('- CON: contractSubject, contractKind, fieldSemantics, validationRules, schemaRef');
  lines.push('- MOD: modulePath, ownedResponsibility, touchWhen[], doNotTouchWhen[], testAnchors');
  lines.push('- VER: verificationGoal, acceptanceOracle, testAnchors');
  lines.push('- OPEN: minimalNextEvidence[], blockedDecisions[], ownerToAsk, escalationGate');
  lines.push('');
  lines.push('Example:');
  lines.push('[{"suggestedType":"CAP","claimText":"Order goods fulfillment lets a customer submit goods as part of an order.","confidence":"high","evidenceRefs":["evidence://entry/EP-001"],"decisionPoints":["requirement_intent"],"sddStageUses":["requirement_clarification"],"unsupportedParts":[],"blockedDecisions":[],"objectHints":{"goal":"Submit goods as part of an order","successCriteria":["Order detail shows submitted goods"],"nonGoals":["Changing payment settlement"]}}]');
  lines.push('');
