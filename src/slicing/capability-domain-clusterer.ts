...
function collectTargetPaths(clusters: FunctionCluster[]): string[] {
  return dedupe(
    clusters.flatMap(cluster => cluster.signals.map(signal => signal.location)),
  ).slice(0, 12);
}

function collectTargetTerms(clusters: FunctionCluster[], nameHint: string): string[] {
  ...
  return dedupe([...raw, ...words]).filter(term => term.length > 1).slice(0, 12);
}

export function buildCapabilityDomainCandidates(functionClusters: FunctionCluster[]): CapabilityDomainCandidate[] {
  ...
  return [...grouped.entries()]
    .map(([key, clusters]) => {
      ...
      return {
        ...
        primaryObjects: dedupe(clusters.map(cluster => cluster.normalizedObject)).slice(0, 6),
        relatedEntities: dedupe(clusters.flatMap(cluster => cluster.domainTerms)).slice(0, 8),
        functionClusters: clusters,
        coreFunctionIds: coreClusters.map(cluster => cluster.clusterId),
        supportingFunctionIds: supportingClusters.map(cluster => cluster.clusterId),
      };
    })
    .filter(candidate =>
      candidate.primaryObjects.some(term => !DOMAIN_STOP_TERMS.has(term)) &&
      candidate.functionClusters.some(cluster => cluster.isCore && cluster.normalizedVerb !== 'query'),
    )
    .sort((left, right) => {
      const coreDiff = right.coreFunctionIds.length - left.coreFunctionIds.length;
