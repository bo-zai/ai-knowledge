...
async function runTool<T extends Record<string, unknown>>(
  ...
  logger.info(
    `[Tool:${options.toolName}] Called with args: ${JSON.stringify(options.args)}`,
  );
  ...
  try {
    ...
    logger.info(
      `[Tool:${options.toolName}] Success: ${result.length} chars in ${duration}ms`,
    );
    return result;
  } catch (err) {
    ...
    const duration = Date.now() - started;
    logger.error(
      `[Tool:${options.toolName}] Failed in ${duration}ms: ${error}`,
    );
    return `Error: ${error}`;
  }
}
...
export function createDomainClusterToolHandlers(
  workspacePath: string,
): DomainClusterToolHandlers {
  // 文件读取
  ...
  // Mapper SQL 获取
  const getMapperStatementsRaw = async (
  ...
  // Controller API 信息获取
  const getControllerApiInfoRaw = async (
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const baseMappingMatch = line.match(
      ...
      for (const [annotation, httpMethod] of methodMappings) {
        if (line.includes(`@${annotation}`)) {
          const pathMatch = line.match(
            new RegExp(`@${annotation}\\s*\\(\\s*(?:value\\s*=\\s*)?"([^"]+)"`),
          );
  ...
  // 外键关系获取（从候选 JSON 解析）
  const getTableForeignKeysRaw = async (
      ...
      for (const file of files.slice(0, 20)) {
        try {
          ...
          if (
            ...
            containsChineseMatch(content, keyword)
          ) {
...
export function createDomainClusterTools(
  workspacePath: string,
  ...
  return [
    new DynamicStructuredTool({
      ...
      schema: z.object({
        ...
        start_line: z
          .number()
          .optional()
          .describe("起始行号（可选，用于分段读取）"),
        end_line: z
          .number()
          .optional()
          .describe("结束行号（可选，用于分段读取）"),
      }),
    ...
    new DynamicStructuredTool({
      ...
      description:
        "获取 Controller 的 REST API 信息。分析 API 路径和业务意图。",
      schema: z.object({
        controller_file_path: z
          .string()
          .describe("Controller Java 文件的绝对路径"),
      }),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        candidate_json: z
          .string()
          .describe("候选的 JSON 字符串（包含 tables 信息）"),
      }),
