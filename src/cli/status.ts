...
export async function runStatus(options: StatusOptions): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Object Types: ${(manifest.object_types as string[])?.join(", ") ?? "unknown"}`,
    );
    ...
    if (coverageExists) {
      ...
      const failures =
        (coverage.failures as Array<Record<string, unknown>>) ?? [];
      ...
      const warnings =
        (coverage.warnings as Array<Record<string, unknown>>) ?? [];
