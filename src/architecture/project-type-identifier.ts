...
export async function identifyProjectType(
  ...
  logger.info(
    `Project type identified: ${context.projectType} (confidence: ${context.confidence})`,
  );
...
export function buildProjectContext(
  ...
  // 处理 snake_case 字段名转换
  const projectType =
    ((raw.projectType ?? raw.project_type) as ProjectContext["projectType"]) ??
    "unknown";
  const primaryLanguage =
    ((raw.primaryLanguage ??
      raw.primary_language) as ProjectContext["primaryLanguage"]) ?? "other";
  ...
  const identificationEvidence =
    ((raw.identificationEvidence ?? raw.identification_evidence) as string[]) ??
    [];
...
function inferProjectTypeFromEvidence(
  ...
  if (
    ...
    evidence.configFiles.some((f) => f.endsWith("pom.xml"))
  ) {
