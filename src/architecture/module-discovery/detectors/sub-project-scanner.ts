...
export class SubProjectScanner extends BaseDetector {
    ...
    try {
      ...
      for (const dir of subDirs) {
        ...
        if (buildConfig) {
          ...
          if (
            buildConfig.type === "java-maven-module" &&
            buildConfig.hasSubModules
          ) {
            const nestedModules = await this.scanMavenModules(
              subDirPath,
              discoveredPaths,
              options?.maxDepth ?? 3,
            );
            ...
            // 单个项目
            const moduleInfo = await this.createModuleInfo(
              dir.name,
              subDirPath,
              buildConfig,
            );
          ...
          // 无构建配置，检查是否是配置目录
          const configModule = await this.detectConfigModule(
            dir.name,
            subDirPath,
          );
          if (configModule) {
  ...
  private async detectBuildConfig(
    dirPath: string,
  ): Promise<{ type: ModuleType; hasSubModules: boolean } | null> {
    // Maven
  ...
  private async scanMavenModules(
    ...
    try {
      ...
      if (!modulesMatch) {
        ...
        const moduleInfo = await this.createModuleInfo(moduleName, modulePath, {
          type: "java-maven-module",
          hasSubModules: false,
        });
      ...
      const subModuleNames = matches.map((m) =>
        m.replace("<module>", "").replace("</module>", "").trim(),
      );
      const modules: ModuleInfo[] = [];
      ...
      for (const subName of subModuleNames) {
        ...
        const moduleInfo = await this.createModuleInfo(subName, subPath, {
          type: "java-maven-module",
          hasSubModules: false,
        });
        if (moduleInfo) {
          ...
          // 递归探测
          if (maxDepth > 1) {
            const nested = await this.scanMavenModules(
              subPath,
              discoveredPaths,
              maxDepth - 1,
            );
  ...
  private async detectRole(
    ...
    if (buildConfig.type === "java-maven-module") {
      try {
        ...
        const hasApp = entries.some(
          (e) => e.isFile() && e.name.includes("Application"),
        );
        if (hasApp) {
          return "deployable";
        }
  ...
  private async extractDependencies(
    ...
    try {
      ...
      const artifactMatches = depsMatch[1].match(
   ...
   * 检测配置目录（无构建配置但有特殊文件）
   */
  private async detectConfigModule(
    name: string,
    dirPath: string,
  ): Promise<ModuleInfo | null> {
