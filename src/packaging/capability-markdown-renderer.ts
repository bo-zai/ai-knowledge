...
export function renderCapabilityMarkdown(
  ...
  if (model.domainName || model.domainKey) {
    ...
    if (conceptLink) {
      lines.push(
        `> 业务域文档：[${model.domainName ?? model.domainKey ?? "业务域"}](${conceptLink})`,
      );
    }
    lines.push("");
  }
  ...
  pushList(
    lines,
    model.includes,
    "当前知识包未提供更细的包含范围，需结合下方行为和代码锚点判断。",
  );
  lines.push("");
  lines.push("不包含范围：");
  pushList(
  ...
  pushList(
    lines,
    model.triggers,
    "当需求提到该能力名称、相关入口、相关数据表或相关模块时，应先阅读本文。",
  );
  lines.push("");
  ...
  if (model.terms.length === 0) {
    lines.push(
      "- 当前知识包没有生成足够可靠的业务术语对象；不要从代码类名直接猜业务含义。",
    );
  } else {
    ...
    for (const term of model.terms) {
      lines.push(
        `| ${escapeCell(term.term)} | ${escapeCell(term.meaningZh)} | ${escapeCell(term.notEqualTo.join(", ") || "-")} | ${escapeCell(evidenceRefs(term.evidenceRefs))} |`,
      );
    }
  }
  lines.push("");
  ...
  if (model.behaviors.length === 0) {
    lines.push(
      "- 当前知识包没有稳定 FLOW 对象；实现前需要从代码入口重建当前行为。",
    );
  } else {
    for (const behavior of model.behaviors) {
      ...
      if (functionLinkPrefix && behavior.functionDocName) {
        lines.push(
          `- [功能文档](${functionLinkPrefix}/${behavior.functionDocName})`,
        );
        lines.push("");
      }
      if (behavior.steps.length > 0) {
        behavior.steps.forEach((step, index) => {
          lines.push(
            `${index + 1}. ${step.step} (${evidenceRefs(step.evidenceRefs)})`,
          );
        });
      } else {
  ...
  if (model.codeAnchors.length === 0) {
    ...
    for (const anchor of model.codeAnchors) {
      lines.push(
        `| ${escapeCell(anchor.role)} | ${escapeCell(anchor.symbolOrRoute)} | ${escapeCell(anchor.path)} | ${escapeCell(anchor.role)} | ${escapeCell(evidenceRefs(anchor.evidenceRefs))} |`,
      );
    }
  }
  lines.push("");
  ...
  if (model.codeAnchors.length === 0) {
    ...
    for (const anchor of model.codeAnchors) {
      ...
      pushList(
        lines,
        anchor.touchWhen,
        "当前知识包没有提供明确 touch_when；修改前需要重新确认模块职责。",
      );
      lines.push("");
      lines.push("不应该修改当：");
      pushList(
        lines,
        anchor.doNotTouchWhen,
        "当前知识包没有提供明确 do_not_touch_when；不要把模块边界当成事实。",
      );
      lines.push("");
    }
  }
  ...
  if (model.dataContracts.length === 0) {
    lines.push(
      "- 当前知识包没有稳定 CON 对象；涉及接口、SQL、表字段或事件时必须补充契约证据。",
    );
  } else {
    for (const contract of model.dataContracts) {
      ...
      if (contract.fields.length > 0) {
        ...
        for (const field of contract.fields) {
          lines.push(
            `| ${escapeCell(field.name)} | ${escapeCell(field.meaningZh || "-")} | ${escapeCell(field.source)} | ${escapeCell(evidenceRefs(field.evidenceRefs))} |`,
          );
        }
      }
  ...
  if (model.unknowns.length === 0) {
    lines.push(
      "- 当前知识包没有 OPEN 对象；这不代表没有未知，遇到 source of truth、验证或外部系统证据缺口仍需停下确认。",
    );
  } else {
    for (const unknown of model.unknowns) {
      ...
      pushList(
        lines,
        unknown.blockedDecisions,
        "当前 OPEN 未声明阻塞决策，使用前需要补充。",
      );
      lines.push("");
      lines.push("最小下一证据：");
      pushList(
  ...
  for (const validation of model.validation) {
    ...
    pushList(
      lines,
      validation.acceptanceOracle,
      "当前知识包没有足够证据给出验收 oracle。",
    );
    lines.push("");
    lines.push("无法验证除非：");
    pushList(
      lines,
      validation.cannotVerifyWithout,
      "已有验证证据足够或不需要额外前置条件。",
    );
    lines.push("");
  ...
  if (model.evidenceIndex.length === 0) {
    lines.push(
      "- 本文没有可展开的 evidence index 条目；请查看 `evidence/index.jsonl` 和 debug 材料。",
    );
  } else {
    ...
    for (const evidence of model.evidenceIndex) {
      lines.push(
        `| ${escapeCell(evidence.ref)} | ${escapeCell(evidence.kind)} | ${escapeCell(evidence.location ?? "-")} | ${escapeCell(evidence.summary ?? evidence.name ?? "-")} |`,
      );
    }
  }
  lines.push("");
