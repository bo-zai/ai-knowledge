...
export async function runConstraintKnowledgePipeline(
  ...
  for (const [index, group] of evidenceGroups.entries()) {
    ...
    logger.info(
      `Generating CONSTRAINT for ${groupId} (${index + 1}/${evidenceGroups.length})`,
    );
  ...
  logger.info(
    `CONSTRAINT pipeline completed: ${succeeded.length} succeeded, ${failed.length} failed`,
  );
...
async function generateConstraintKnowledge(
  ...
  try {
    ...
    logger.warn(
      `Schema validation failed for CONSTRAINT ${groupId}, using fallback`,
    );
    const fallbackContent = generateFallbackConstraint(group);
    await writeFile(filePath, fallbackContent);
    return { groupId, success: true, filePath, content: fallbackContent };
 ...
 * 约束知识转 Markdown
 */
function constraintToMarkdown(
  data: ConstraintKnowledge,
  group: EvidenceGroup,
): string {
  const lines: string[] = [];
