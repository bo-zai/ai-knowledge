...
export async function runWorkflowKnowledgePipeline(
  ...
  const knowledgeDir = path.join(outputRoot, 'ai-knowledge', TYPE_TO_DIR['WORKFLOW']);
  const results: WorkflowGenerationResult[] = [];

  for (const [index, group] of evidenceGroups.entries()) {
    const groupId = group.groupId;
    logger.info(`Generating WORKFLOW for ${groupId} (${index + 1}/${evidenceGroups.length})`);
  ...
  logger.info(`WORKFLOW pipeline completed: ${succeeded.length} succeeded, ${failed.length} failed`);
...
async function generateWorkflowKnowledge(
  ...
  try {
    ...
    logger.warn(`Schema validation failed for WORKFLOW ${groupId}, using fallback`);
    const fallbackContent = generateFallbackWorkflow(group);
    await writeFile(filePath, fallbackContent);
    return { groupId, success: true, filePath, content: fallbackContent };
  }
 ...
 * 跨域业务流程知识转 Markdown
 */
function workflowToMarkdown(data: WorkflowKnowledge, group: EvidenceGroup): string {
  const lines: string[] = [];
  ...
  for (const step of data.steps) {
    lines.push(`| ${step.order} | ${step.domain} | ${step.action} | ${step.description} |`);
  }
  lines.push('');
