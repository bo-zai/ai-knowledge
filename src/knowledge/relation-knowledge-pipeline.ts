...
export async function runRelationKnowledgePipeline(
  ...
  for (const [index, group] of evidenceGroups.entries()) {
    ...
    logger.info(
      `Generating RELATION for ${groupId} (${index + 1}/${evidenceGroups.length})`,
    );
  ...
  logger.info(
    `RELATION pipeline completed: ${succeeded.length} succeeded, ${failed.length} failed`,
  );
...
async function generateRelationKnowledge(
  ...
  try {
    ...
    logger.warn(
      `Schema validation failed for RELATION ${groupId}, using fallback`,
    );
    const fallbackContent = generateFallbackRelation(group);
    await writeFile(filePath, fallbackContent);
    return { groupId, success: true, filePath, content: fallbackContent };
 ...
 * 能力关系知识转 Markdown
 */
function relationToMarkdown(
  data: RelationKnowledge,
  group: EvidenceGroup,
): string {
  const lines: string[] = [];
