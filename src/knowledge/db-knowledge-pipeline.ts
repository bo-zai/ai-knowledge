...
function parseSliceFilter(value?: string): SliceFilter | null {
  ...
  if (knownKinds.includes(prefix as SliceKind) && rest) {
    return { kind: prefix as SliceKind, raw: normalized, target: rest.trim().toLowerCase() };
  }
...
function countSlicesByKind(slices: SliceSeed[]): Record<SliceKind, number> {
  return slices.reduce<Record<SliceKind, number>>(
    (counts, slice) => { counts[slice.kind] += 1; return counts; },
...
function inferObjectType(sliceKind: string): 'CON' | 'FLOW' | 'MOD' | 'TERM' | 'DB' | 'OWN' | 'VER' | 'OPEN' {
  const mapping: Record<string, 'CON' | 'FLOW' | 'MOD' | 'TERM' | 'DB'> = {
    route: 'CON', process: 'FLOW', tool: 'MOD', community: 'TERM', database: 'DB',
  };
...
function getPromptBuilderForSliceKind(kind: string): ((input: unknown) => { system: string; user: string }) | null {
  ...
  const builders: Record<string, (input: unknown) => { system: string; user: string }> = {
    route: buildConPrompt, process: buildFlowPrompt, tool: buildModPrompt, community: buildTermPrompt,
  };
...
function validateObject(draft: unknown, objectType: string): unknown | null {
  try {
    switch (objectType) {
      case 'DB': return dbObjectSchema.parse(draft);
      case 'CON': return conObjectSchema.parse(draft);
      case 'TERM': return termObjectSchema.parse(draft);
      case 'FLOW': return flowObjectSchema.parse(draft);
      case 'MOD': return modObjectSchema.parse(draft);
      case 'OPEN': return openObjectSchema.parse(draft);
      case 'OWN': return ownObjectSchema.parse(draft);
      case 'VER': return verObjectSchema.parse(draft);
      default: return null;
    }
  } catch { return null; }
}
...
function stripCommonFields(object: Record<string, unknown>): Record<string, unknown> {
  const {
    id: _id, type: _type, title: _title, status: _status, maturity: _maturity,
    scope: _scope, repo: _repo, slice_ids: _sliceIds, evidence_primary: _evidencePrimary,
    evidence_secondary: _evidenceSecondary, stale_if: _staleIf, generated_by: _generatedBy,
    generated_at: _generatedAt, ...rest
  } = object;
  return rest;
}

function renderObjectMarkdown(frontmatter: Record<string, unknown>, body: string): string {
  const fmLines = ['---'];
...
function renderConMarkdown(validated: ReturnType<typeof conObjectSchema.parse>): string {
  const frontmatter = {
  ...
  return renderObjectMarkdown(frontmatter, YAML.stringify(stripCommonFields(validated as Record<string, unknown>)));
}
  ...
  switch (slice.kind) {
    case 'route': {
      ...
      return buildRouteSliceEvidence({
        route: slice.title, handlerFile: `${repoPath}/src/routes/${routePath.replace(/\//g, '_')}.ts`,
        method, path: routePath, middleware: [], responseShape: [], errorShape: [], tests: [],
      });
    }
    case 'process':
      return buildProcessSliceEvidence({
        ...
        steps: [{ order: 1, action: 'start', actor: 'system', file: `${repoPath}/src/processes/${slice.title.toLowerCase()}.ts` }],
        outcomes: [], errorHandling: [],
      });
    case 'tool':
      return buildModuleSliceEvidence({
        moduleName: slice.title, filePath: `${repoPath}/src/tools/${slice.title.toLowerCase()}.ts`,
        exports: [{ name: slice.title, kind: 'function' }], imports: [], dependsOn: [], usedBy: [],
      });
    case 'database': {
      const dbBundle = dbBundleMap.get(slice.title.toLowerCase());
      if (dbBundle) {
        return buildDatabaseSliceEvidence({
          tableName: dbBundle.table, schemaName: 'public',
          sourceFile: dbBundle.mapperBindings[0]?.mapperFile ?? repoPath, sourceKind: 'orm',
          fields: dbBundle.fieldCandidates.length > 0
            ? dbBundle.fieldCandidates.map((field) => ({
                name: field.name, type: field.type ?? 'unknown', description: field.name,
                source: field.source === 'inferred' ? 'inferred' : 'comment',
              }))
            : [{ name: 'id', type: 'unknown', description: 'id', source: 'inferred' }],
          primaryKey: [], foreignKeys: [],
          readBy: dbBundle.mapperBindings.filter((b) => b.statementType === 'select').map((b) => `${b.namespace}.${b.methodId}`),
          writeBy: dbBundle.mapperBindings.filter((b) => b.statementType !== 'select').map((b) => `${b.namespace}.${b.methodId}`),
        });
      }
      return buildDatabaseSliceEvidence({
        tableName: slice.title, schemaName: 'public', sourceFile: `${repoPath}/src/db/schema/${slice.title}.ts`,
        sourceKind: 'orm', fields: [], primaryKey: [], foreignKeys: [], readBy: [], writeBy: [],
      });
    }
    case 'community':
      return {
        slice: { id: slice.id, kind: 'community', title: slice.title, scope: slice.title, seed: slice.title },
        facts: [{ id: 'F-TERM-001', claim: `术语 ${slice.title} 在仓库中被使用`, source_kind: 'analysis-runtime', refs: [{ file: repoPath }] }],
        symbols: [], relations: [], snippets: [], tables: [], tests: [],
        gaps: [{ id: 'G-TERM-001', kind: 'missing-definition', question: `术语 ${slice.title} 的具体定义是什么？`, reason: '未从代码中提取到术语定义' }],
      };
    default:
      return {
        slice: { id: slice.id, kind: 'route', title: slice.title, scope: slice.id, seed: slice.id },
        facts: [], symbols: [], relations: [], snippets: [], tables: [], tests: [],
        gaps: [{ id: `G-${slice.id}-001`, kind: 'unknown-slice-kind', question: `如何处理 ${slice.kind} 类型的切片？`, reason: `未知的切片类型: ${slice.kind}` }],
      };
  }
}
...
async function generateObjectForSlice(
  ...
  if (!promptBuilder) {
    ...
    return {
      objectResult: null,
      trace: { sliceId: slice.id, sliceKind: slice.kind, sliceTitle: slice.title, objectType: 'OPEN', mode: isMockModel(model) ? 'mock' : 'llm', status: 'error', startedAt: now, finishedAt: now, durationMs: 0, request: { systemPrompt: '', userPrompt: '' }, response: {}, error: `No generator for slice kind ${slice.kind}` },
    };
  }
  ...
  const output = isMockModel(model)
    ...
    : await withRetry(
        ...
        return {
          objects: [], warnings: [],
          __generationError: { message: errorMessage, finishedAt: finishedAt.toISOString(), durationMs: finishedAt.getTime() - startedAt.getTime() },
        } as GeneratorOutput & { __generationError: GenerationErrorMeta };
      });

  if (isMockModel(model)) { parsedOutput = output; rawText = JSON.stringify(output); }

  if ('__generationError' in output) {
    const generationError = output.__generationError as GenerationErrorMeta;
    return {
      objectResult: null,
      trace: { sliceId: slice.id, sliceKind: slice.kind, sliceTitle: slice.title, objectType, mode: isMockModel(model) ? 'mock' : 'llm', status: 'error', startedAt: startedAt.toISOString(), finishedAt: generationError.finishedAt, durationMs: generationError.durationMs, request: { systemPrompt: system, userPrompt: user }, response: { rawText, parsedOutput, warnings: [], llm: llmResult }, error: generationError.message },
    };
  }
  ...
  if (output.objects.length === 0) {
    return {
      objectResult: null,
      trace: { sliceId: slice.id, sliceKind: slice.kind, sliceTitle: slice.title, objectType, mode: isMockModel(model) ? 'mock' : 'llm', status: 'empty', startedAt: startedAt.toISOString(), finishedAt: finishedAtForOutput.toISOString(), durationMs: finishedAtForOutput.getTime() - startedAt.getTime(), request: { systemPrompt: system, userPrompt: user }, response: { rawText, parsedOutput, warnings: output.warnings, llm: llmResult } },
    };
  }
  ...
  const frontmatter = {
    id: objectId, type: objectType, title: slice.title, status: 'fact', maturity: 'bootstrap',
    scope: slice.id, repo: getRepoBasename(repoPath), slice_ids: [slice.id],
    evidence_primary: [slice.id], evidence_secondary: [], stale_if: [],
    generated_by: 'repo-knowledge-generator', generated_at: generatedAt,
  };

  const candidateObject = { ...(typeof draft === 'object' && draft !== null ? draft : {}), ...frontmatter, id: objectId, type: objectType };
  const validated = validateObject(candidateObject, objectType);
  if (!validated) {
    const finishedAt = new Date();
    return {
      objectResult: null,
      trace: { sliceId: slice.id, sliceKind: slice.kind, sliceTitle: slice.title, objectType, mode: isMockModel(model) ? 'mock' : 'llm', status: 'validation_failed', startedAt: startedAt.toISOString(), finishedAt: finishedAt.toISOString(), durationMs: finishedAt.getTime() - startedAt.getTime(), request: { systemPrompt: system, userPrompt: user }, response: { rawText, parsedOutput, warnings: output.warnings, llm: llmResult }, validation: { passed: false, error: 'Schema validation failed' } },
    };
  }
  ...
  if (objectType === 'CON' && validated) {
    ...
    content = renderObjectMarkdown(frontmatter, YAML.stringify(stripCommonFields(validated as Record<string, unknown>)));
  }

  const finishedAt = new Date();
  return {
    objectResult: { id: objectId, type: objectType, content, frontmatter },
    trace: { sliceId: slice.id, sliceKind: slice.kind, sliceTitle: slice.title, objectType, mode: isMockModel(model) ? 'mock' : 'llm', status: 'success', startedAt: startedAt.toISOString(), finishedAt: finishedAt.toISOString(), durationMs: finishedAt.getTime() - startedAt.getTime(), request: { systemPrompt: system, userPrompt: user }, response: { rawText, parsedOutput, warnings: output.warnings, llm: llmResult }, validation: { passed: true } },
  };
}
...
function buildMockGeneratorOutput(
  ...
  const payload = (evidence ?? {}) as { slice?: SliceEvidenceBundle; db_bundle?: DbTableEvidenceBundle | null };
  const baseWarnings = [{ message: 'mock-model-output' }];

  switch (objectType) {
    case 'DB': {
      const dbBundle = payload.db_bundle;
      const fields = dbBundle && dbBundle.fieldCandidates.length > 0
        ? dbBundle.fieldCandidates.map((field) => ({
            name: field.name, type: field.type ?? 'unknown', nullable: true, default: null,
            description_zh: field.javaFieldComment || field.mappedJavaProperty || field.name,
            description_source: field.javaFieldComment ? 'comment' : 'inferred', constraints: [],
          }))
        : [{ name: 'id', type: 'unknown', nullable: true, default: null, description_zh: '主键', description_source: 'inferred', constraints: [] }];
      return {
        objects: [{
          table_name: dbBundle?.table ?? slice.title, table_name_zh: `${dbBundle?.table ?? slice.title}表`,
          schema_name: 'public', source_kind: 'mapper', primary_key: [], indexes: [], foreign_keys: [],
          read_by_direct: dbBundle?.directStatements.filter((s) => s.statementType === 'select').map((s) => s.id) ?? [],
          read_by_joined: dbBundle?.joinedStatements.filter((s) => s.statementType === 'select').map((s) => s.id) ?? [],
          write_by_direct: dbBundle?.directStatements.filter((s) => s.statementType !== 'select').map((s) => s.id) ?? [],
          write_by_joined: dbBundle?.joinedStatements.filter((s) => s.statementType !== 'select').map((s) => s.id) ?? [],
          fields, gaps: [],
        }],
    ...
    case 'CON':
      return { objects: [{ interface_kind: 'route', interface_name: slice.title, interface_name_zh: `${slice.title}接口`, producer: slice.title, producer_zh: '路由处理器', consumers: [], input_shape: [], input_description_zh: '当前未提取到输入字段', output_shape: [], output_description_zh: '当前未提取到输出字段', middleware: [], related_routes: [slice.title], related_tools: [], entry_file: payload.slice?.facts[0]?.refs[0]?.file ?? slice.title }], warnings: baseWarnings };
    case 'FLOW':
      return { objects: [{ flow_name: slice.title, flow_name_zh: `${slice.title}流程`, trigger: slice.id, trigger_zh: '触发该流程的入口', steps: [{ order: 1, action: 'start', action_zh: '开始处理', actor: 'system', inputs: [], outputs: [] }], outcomes: [], error_handling: [] }], warnings: baseWarnings };
    case 'MOD':
      return { objects: [{ module_name: slice.title, module_name_zh: `${slice.title}模块`, responsibility_zh: '承载当前切片相关逻辑', module_kind: 'utility', exports: [], imports: [], depends_on: [], used_by: [] }], warnings: baseWarnings };
    case 'TERM':
      return { objects: [{ term: slice.title, term_zh: slice.title, definition_zh: `术语 ${slice.title} 在当前仓库中被使用`, aliases: [], related_terms: [], used_in: [slice.id] }], warnings: baseWarnings };
    case 'OPEN':
      return { objects: [{ question: `${slice.title} 仍有待确认信息`, question_zh: `${slice.title} 仍有待确认信息`, context_zh: 'mock 模式下未连接真实模型，保留开放问题', impact: 'medium' }], warnings: baseWarnings };
    case 'OWN':
      return { objects: [{ owner_type: 'shared', owner_name: slice.title, owner_name_zh: slice.title, responsibility_zh: '当前切片的默认责任边界', scope_items: [slice.id] }], warnings: baseWarnings };
    case 'VER':
      return { objects: [{ version_name: slice.title, version_name_zh: slice.title, version_value: 'bootstrap', version_kind: 'tool', changelog_zh: 'mock 模式下生成的占位版本信息', breaking_changes: [] }], warnings: baseWarnings };
  }
}
...
export async function runDbKnowledgePipeline(
  ...
  if (slicePlan.total_count === 0) {
    ...
    return {
      ...
      files: [],
      objects: [],
      report: buildDbStageReport({ succeeded: 0, failed: 0, targetTable: target?.value }),
      warnings: ['No slices matched the current filters'],
    };
  }

  // 4. Build repo evidence
  const repoEvidence = buildRepoEvidenceBundle({ repoPath, repoName: getRepoBasename(repoPath) });
  ...
  for (const [index, slice] of slicePlan.slices.entries()) {
    ...
    try {
      ...
      const generation = await generateObjectForSlice(slice, combinedEvidence, client, modelConfig.model, repoPath);
      ...
      if (generation.objectResult) {
        ...
        failures.push({
          ...
          error: generation.trace.validation?.error ?? generation.trace.error ?? 'Object generation failed',
        });
      }
      logger.info(`Finished ${sliceProgress} with status ${generation.trace.status} in ${generation.trace.durationMs}ms`);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      const failedObjectType = inferObjectType(slice.kind);
      failures.push({ id: generateObjectId(failedObjectType, slice.id), type: failedObjectType, error: errorMsg });
      debugTraces.push({
        sliceId: slice.id, sliceKind: slice.kind, sliceTitle: slice.title, objectType: failedObjectType,
        mode: mockMode ? 'mock' : 'llm', status: 'error',
        startedAt: new Date().toISOString(), finishedAt: new Date().toISOString(), durationMs: 0,
        request: { systemPrompt: '', userPrompt: '' }, response: {}, error: errorMsg,
      });
  ...
  if (debugTraces.length > 0) {
    ...
    files.push({
      ...
      content: JSON.stringify({ repoId, model: modelConfig.model, traces: debugTraces }, null, 2) + '\n',
    });
  }
  ...
  const objects = generatedObjects.map((obj) => ({
    ...
    type: obj.type,
    path: getObjectPath(obj.type, obj.id),
    sliceIds: Array.isArray(obj.frontmatter.slice_ids) ? (obj.frontmatter.slice_ids as string[]) : [],
  }));
  ...
  return {
    ...
    warnings: [...warnings, ...failures.map((f) => `[${f.type}] ${f.id}: ${f.error}`)],
  };
}
