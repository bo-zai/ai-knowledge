...
import { buildAllAggregateBundles, type DataModelAggregateBundle } from '../evidence/data-model-bundle-builder.js';
import { buildDataModelPrompt } from '../generation/object-generators/data-model-generator.js';
import { dataModelObjectSchema, type DataModelObject } from '../schemas/data-model.js';
...
export async function runDataModelKnowledgePipeline(
  ...
  const { repoPath, modelConfig, claimsProvider, outputRoot, companionRepoPath, conceptNames, timeout } = input;

  logger.info('Starting DATA_MODEL knowledge pipeline...');

  // 1. 构建聚合证据包
  logger.info('Building aggregate evidence bundles...');
  const aggregateBundles = await buildAllAggregateBundles(repoPath, companionRepoPath);
  ...
  const knowledgeDir = path.join(outputRoot, 'ai-knowledge', TYPE_TO_DIR['DATA_MODEL']);

  for (const [index, bundle] of aggregateBundles.entries()) {
    const aggregateName = bundle.suggested_aggregate_name;
    logger.info(`Generating DATA_MODEL for ${aggregateName} (${index + 1}/${aggregateBundles.length})`);
  ...
  logger.info(`DATA_MODEL pipeline completed: ${succeeded.length} succeeded, ${failed.length} failed`);
  ...
  const objects = succeeded.map((r) => ({
    ...
    type: 'DATA_MODEL',
    path: path.join(TYPE_TO_DIR['DATA_MODEL'], `${toFileName(r.aggregateName)}.md`),
    sliceIds: [],
  }));
...
async function generateAggregateKnowledge(
  ...
  try {
    ...
    logger.warn(`Schema validation failed for ${aggregateName}, using fallback`);
    const fallbackContent = generateFallbackDataModel(bundle);
    await writeFile(filePath, fallbackContent);
    return { aggregateName, success: true, filePath, content: fallbackContent };
  }
 ...
 * 聚合知识转 Markdown
 */
function dataModelToMarkdown(data: DataModelObject, bundle: DataModelAggregateBundle): string {
  const lines: string[] = [];
  ...
  lines.push(`> 标签：${data.aggregate_name_zh?.replace('聚合', '') || '数据模型'}、聚合、数据模型`);
  lines.push('');
  ...
  for (const entity of data.entities) {
    ...
    lines.push(`| ${entity.entity_name} | ${entity.description_zh} | ${roleZh} |`);
  }
  lines.push('');
  ...
  if (data.entity_relations.length > 0) {
    ...
    for (const relation of data.entity_relations) {
      ...
      lines.push(`- **${relation.source_entity} → ${relation.target_entity}**：${typeZh}（关联字段：${relation.relation_field}）`);
    }
    lines.push('');
  }
  ...
  if (data.cross_references && data.cross_references.length > 0) {
    ...
    for (const ref of data.cross_references) {
      lines.push(`- 引用「${ref.aggregate_name}」中的 ${ref.entity_name}（通过 ${ref.reference_field}）`);
    }
    lines.push('');
  }
  ...
  lines.push(`## 标签`);
  lines.push('');
  lines.push(`${data.aggregate_name_zh?.replace('聚合', '') || '数据模型'}、聚合、数据模型`);
...
function generateFallbackDataModel(bundle: DataModelAggregateBundle): string {
  ...
  for (const entity of bundle.entities) {
    ...
    lines.push(`| ${entity.entity_name} | ${entity.entity_name_zh} | ${roleZh} |`);
  }
  lines.push('');
