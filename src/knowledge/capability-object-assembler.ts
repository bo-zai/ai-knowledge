...
): "llm" | "skeleton" | "evidence_seed" {
  return claim.source ?? "llm";
}

function extractBehaviorRef(
  claim: CandidateClaim,
  bundle: EvidenceBundle,
): EvidenceBehaviorSlice | undefined {
...
function extractFlowRef(
  claim: CandidateClaim,
  bundle: EvidenceBundle,
): EvidenceFlowTrace | undefined {
  for (const ref of claim.evidenceRefs) {
...
function extractModuleRef(
  claim: CandidateClaim,
  bundle: EvidenceBundle,
): EvidenceModuleSurface | undefined {
  for (const ref of claim.evidenceRefs) {
...
function extractContractRef(
  claim: CandidateClaim,
  bundle: EvidenceBundle,
): EvidenceDataContract | undefined {
  for (const ref of claim.evidenceRefs) {
...
function extractValidationRef(
  claim: CandidateClaim,
  bundle: EvidenceBundle,
): EvidenceValidationAnchor | undefined {
  for (const ref of claim.evidenceRefs) {
...
function buildTermObject(claim: CandidateClaim): KnowledgeObject {
  ...
  return {
    ...
    metadata: {
      source: claimSource(claim),
      canonicalTerm,
      ...(hints?.businessDefinition
        ? { businessDefinition: hints.businessDefinition }
        : {}),
...
function buildFlowObject(
  ...
  return {
    ...
    metadata: {
      ...
      ...(hints?.failureBranches
        ? { failureBranches: hints.failureBranches }
        : {}),
...
function buildModObject(
  ...
  return {
    ...
    metadata: {
      source: claimSource(claim),
      rootPath: path,
      ...(hints?.ownedResponsibility
        ? { ownedResponsibility: hints.ownedResponsibility }
        : {}),
      ...(entryPoints.length > 0 ? { entryPoints } : {}),
      ...(hints?.touchWhen ? { touchWhen: hints.touchWhen } : {}),
      ...(hints?.doNotTouchWhen
        ? { doNotTouchWhen: hints.doNotTouchWhen }
        : {}),
...
function extractModEntrySymbols(
  claim: CandidateClaim,
  bundle: EvidenceBundle,
  modulePath: string,
): string[] {
  ...
  for (const beh of bundle.behaviorSlices) {
    ...
    if (matchesModulePath(locLower, pathLower)) {
      const className =
        beh.location.split("/").pop()?.replace(".java", "") || "";
    ...
    return {
      ...
      metadata: {
        ...
        ...(hints?.fieldSemantics
          ? { fieldSemantics: hints.fieldSemantics }
          : {}),
        ...(hints?.validationRules
          ? { validationRules: hints.validationRules }
          : {}),
      },
    };
  }
...
function extractApiRouteInfo(contract: EvidenceDataContract): {
  method: string;
  path: string;
...
function buildVerObject(
  ...
  return {
    ...
    metadata: {
      ...
      ...(hints?.verificationGoal
        ? { verificationGoal: hints.verificationGoal }
        : {}),
      ...(hints?.acceptanceOracle
        ? { acceptanceOracle: hints.acceptanceOracle }
        : {}),
      ...(hints?.testAnchors ? { testAnchors: hints.testAnchors } : {}),
    },
  };
...
function buildOpenObject(claim: CandidateClaim): KnowledgeObject {
  const hints = claim.objectHints;
  const question = claim.claimText
    .slice(0, 50)
    .replace(/[^A-Z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return {
    ...
    metadata: {
      source: claimSource(claim),
      ...(hints?.minimalNextEvidence
        ? { minimalNextEvidence: hints.minimalNextEvidence }
        : {}),
      ...(hints?.ownerToAsk ? { ownerToAsk: hints.ownerToAsk } : {}),
      ...(hints?.escalationGate
        ? { escalationGate: hints.escalationGate }
        : {}),
    },
  };
}
