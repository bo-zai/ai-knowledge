...
Agent 读取知识库时第一个打开的文件。它提供两个视角的知识导航：

- **业务域导航**：按业务域聚合跨类型知识引用，使 Agent 能从一个业务域出发一次定位所有相关知识文件
- **按类型索引**：按知识类型列出所有条目概要，使 Agent 能扫描某个类型下的全部条目
...
- 业务域导航表位于架构概览之后、按类型表格之前，是 Agent 按业务域检索知识的主入口
- 业务域导航表的每行聚合该域关联的能力、概念、约束和数据聚合的文件链接，无关联时填"—"
- 单个业务域关联的概念或约束超过 3 条时，只列出引用频次最高的 3 条
- 多模块项目的业务域导航表增加"模块"列，标注该域涉及的主要模块
- 按类型表格中每条概要严格一行（约 20~30 token），足以让 Agent 判断是否需要读详情
- 总 token 量随项目规模弹性增长：小型项目（≤ 10 条目）约 1000~1500 token，中型项目（10~50 条目）约 2000~3500 token，大型项目（50+ 条目）不超过 5000 token
...
其他类型的 `_index.md` 结构相同，只是第一列名称和辅助信息列不同（不含别名列）。以约束知识为例：
...
详细能力清单参见 [能力目录](capabilities/_index.md)
...
- **业务领域全景**将业务域分为核心域、支撑域和辅助域，说明域间主要交互方向，链接到能力目录获取详细清单。不在此列出具体业务模块的包路径和类名
...
### modules.json：模块拓扑（紧耦合多模块项目）

仅紧耦合多模块项目需要此文件。松耦合模式和单模块项目不需要。

modules.json 记录仓库内各模块的路径、角色、依赖关系和入口，供 Agent 理解代码组织方式和生成程序执行模块维度的增量更新。
  ...
  "modules": [
    {
      ...
      "role": "deployable",
      "description": "后台管理系统，提供商品、订单、用户等管理 API",
      "dependencies": ["mall-common", "mall-mbg", "mall-security"],
      "entryPoint": "mall-admin/src/main/java/com/macro/mall/MallAdminApplication.java",
      "packageRoot": "com.macro.mall"
    },
    {
      "name": "mall-mbg",
      "path": "mall-mbg",
      "type": "library",
      "role": "shared",
      "description": "数据访问层，MyBatis Generator 生成的实体类和 Mapper",
      "dependencies": [],
      "usedBy": ["mall-admin", "mall-portal", "mall-search"],
      "packageRoot": "com.macro.mall"
    }
  ]
}
...
- Agent 理解仓库的模块组织方式和模块间依赖关系
- 生成程序执行增量更新时，沿模块依赖图传播变更影响
- Agent 定位代码时，知道某个能力属于哪个模块、在哪个目录下
- Agent 分析变更影响时，知道共享模块的变更会影响哪些服务
...
多模块项目的知识库不按 `ai-knowledge/{module}/capabilities/` 的方式组织，而是保持扁平结构并在条目中标注模块归属，原因：

- **业务知识按业务域内聚，不按模块内聚**。Agent 的主要检索模式是"按业务域找知识"（如"订单相关的所有知识"），而非"按模块找知识"（如"mall-admin 里的所有知识"）。按模块拆分会导致同一业务域的知识碎片化在多个模块目录下
- **共享知识无处安放**。定义在共享模块中的实体（如 mall-mbg 的 OmsOrder）被多个服务使用，按模块组织时要么重复生成，要么需要额外的引用机制
- **跨域业务流程无法按模块组织**。一条跨模块的端到端流程无法自然归入任何一个模块目录
- **Agent 检索路径变长**。按模块组织增加了一层导航（先选模块，再选知识类型），且"判断需求属于哪个模块"本身就需要业务知识
- **模块信息用于过滤，不用于组织**。模块标注（"这个能力属于 mall-admin"）作为元数据字段存在于条目中，Agent 可以按模块过滤，但不需要在目录树上先选模块

### 为什么松耦合模式使用独立知识库而非统一知识库

松耦合多模块项目（各服务独立数据库、无直接代码调用）使用各服务目录下独立的 `ai-knowledge/`，而非仓库级统一知识库，原因：

- **各服务是独立系统**。它们不共享业务实体、不共享数据库、不共享业务流程，没有跨服务知识共享的需求
- **知识条目不需要模块标注**。每个知识库天然对应一个服务，模块信息是隐含的
- **增量更新完全隔离**。一个服务的代码变更不影响其他服务的知识库
- **独立团队独立维护**。松耦合服务通常由不同团队负责，独立知识库与团队边界对齐
